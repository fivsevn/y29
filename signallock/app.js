...
function restart(){if(s.phase==='ended'||actions.length===0){reset();return;}show('RECONNECT / 重新连接','<p>开始新的连接会清除本局进度，已发现的结局会保留。</p><div class="actions dialog-actions"><button data-command="cancel"><span class="dialog-command"><span class="keycap">ESC</span><span class="cmd-label">KEEP SESSION</span></span><small>继续当前连接</small></button><button data-command="confirm-restart" class="primary"><span class="dialog-command"><span class="keycap">ENTER</span><span class="cmd-label">RECONNECT</span></span><small>确认重新连接</small></button></div>');}
...
$('sound').onclick=()=>{soundOn=!soundOn;soundLabel();$('sound').setAttribute('aria-pressed',String(soundOn));beep();};
