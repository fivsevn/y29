import {normalizeState,currentView,wrapMessage,POLL_MS} from './state.js';
const el = id => document.getElementById(id);
const endpoint = new URLSearchParams(location.search).get('state') || './current-state.json';
...
function render(){
  ...
  el('source').textContent='FROM / '+view.source;
  el('message').textContent=wrapMessage(view.message || '等待当前消息。');
